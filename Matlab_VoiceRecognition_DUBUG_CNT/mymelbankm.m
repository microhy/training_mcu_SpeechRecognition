...
if ~any(w=='H')
    switch wr
        ...
        otherwise
            mflh=myfrq2mel(mflh);       % convert frequency limits into mel
    end
end
...
switch wr
    ...
    otherwise
        blim=mymel2frq(mflh(1)+[0 1 p p+1]*melinc)*n/fs;
end
...
switch wr
    ...
    otherwise
        pf=(myfrq2mel((b1:b4)*fs/n)-mflh(1))/melinc;
end
%
...
if ~nargout || any(w=='g') % plot idealized filters
    ...
    switch wr
        ...
        otherwise
            fe=mymel2frq(me); % defining frequencies
            xg=mymel2frq(repmat(linspace(0,1,ng),p,1).*repmat(me(3:end)-me(1:end-2),1,ng)+repmat(me(1:end-2),1,ng));
    end
